export async function awaitPromisseOneSecond(timeMiliSeconds: number) {
    await new Promise(resolve => setTimeout(resolve, timeMiliSeconds))
}

export async function createLoading(element: HTMLElement, timeMiliSeconds: number = 1000) {
    element.innerHTML += `
